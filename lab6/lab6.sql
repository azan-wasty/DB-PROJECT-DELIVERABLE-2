...
CREATE VIEW SJWD AS
SELECT D.DungeonID, Name, Difficulty, Boss, Status
FROM Dungeons D JOIN Hunter_Missions HM ON D.DungeonID = HM.DungeonID
WHERE HM.HunterID = 1;
...
GO
CREATE VIEW BLC AS
SELECT BeyonderID, B.Name, Path, PotionConsumption, Status, O.Name AS OrganizationName
FROM Beyonders B JOIN Organizations O ON B.OrganizationID = O.OrganizationID
WHERE Status = 'Lost Control' OR Status = 'At Risk' 
GO

--Q3
GO
CREATE VIEW Q3 AS
SELECT M.MissionID, Title, Type, DifficultyLevel, Status, O.Name AS OrganizationName
FROM Missions M LEFT JOIN Mission_Assignments MA ON M.MissionID = MA.MissionID 
LEFT JOIN Agents A ON MA.AgentID = A.AgentID 
...
GO
CREATE VIEW Q4 AS
SELECT HM.MissionID, M.Title, COUNT(HM.MissionStatus) AS TimesFailed, H.Name AS MostCommonHunter, B.Name AS MostCommonBeyonder, A.Name AS HighRiskArtifact
FROM Hunter_Missions HM JOIN Missions M ON HM.MissionID = M.MissionID
JOIN Mission_Assignments MA ON HM.MissionID = MA.MissionID
JOIN Hunters H ON MA.HunterID = H.HunterID
LEFT JOIN Artifacts A ON M.MissionID = A.MissionID
LEFT JOIN Beyonders B ON B.BeyonderID = A.GuardianID

WHERE HM.MissionStatus = 'Failed' AND A.RiskLevel = 'High'-- AND B.Status = 'At Risk'
GROUP BY HM.MissionID, M.Title, H.Name, B.Name, A.Name 
ORDER BY COUNT(HM.MissionStatus) DESC
GO

--Q5
GO
CREATE VIEW Q5 AS
SELECT HunterID, H.Name, Guild, PowerLevel, O.Name AS OrganizationName
FROM Hunters H JOIN Organizations O ON H.OrganizationID = O.OrganizationID
WHERE H.Rank = 'S'
GO
...
SELECT * FROM Hunter_Missions
SELECT * FROM Beyonders
